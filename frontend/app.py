...
# ── Path Setup ────────────────────────────────────────────────────────────────
# Ensure both the project root (for backend/agents imports) and the frontend
# directory (for utils/pages/components imports) are on sys.path, regardless
# of which working directory `streamlit run` is invoked from.
_here = os.path.dirname(os.path.abspath(__file__))          # .../frontend
_project_root = os.path.abspath(os.path.join(_here, ".."))  # .../LexFusion-share
